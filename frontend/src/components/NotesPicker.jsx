...
const NotesPicker = () => {
  ...
  const fetchNotes = async () => {
    try {
      const response = await fetch('https://edutimely-backend.onrender.com/api/notes');
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://edutimely-backend.onrender.com/api/notes/${id}`, { method: 'DELETE' });
      fetchNotes();
    } catch (error) {
  ...
  const handleRename = async (id) => {
    ...
    try {
      const response = await fetch(`https://edutimely-backend.onrender.com/api/notes/${id}`, {
        method: 'PUT',
  ...
  <center><button type="submit" className="upload-button">Upload</button></center>
      ...
      {contextMenu && (
        <ul className="context-menu" style={{ top: contextMenu.y, left: contextMenu.x }}>
          <li>
            <a href={`https://edutimely-backend.onrender.com${contextMenu.note.fileUrl}`} target="_blank" rel="noopener noreferrer">
              View File
            </a>
          </li>
