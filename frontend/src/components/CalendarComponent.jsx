...
const CalendarComponent = ({ userId }) => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.get(`https://edutimely-backend.onrender.com/api/calendar-events/${userId}`);
  ...
  const handleAddEvent = async () => {
    ...
    try {
      await axios.post(`https://edutimely-backend.onrender.com/api/calendar-events`, { ...newEvent, userId });
      fetchEvents();
  ...
  const handleDeleteEvent = async (id) => {
    try {
      await axios.delete(`https://edutimely-backend.onrender.com/api/calendar-events/${id}`);
